    ...
    if self.tileData.destructible and not UI.active then
        ...
        if self.hp < 0 then
            ...
            if self.tileData.type == "Grass" then
                nextType = convertIconToDefinition("Dirt")
            end
        
            if self.tileData.type == "Dirt" then
                nextType = convertIconToDefinition("Wall")
            end
...
function entity:onInteract(entity)
    if self.tileData.interactable then
        if entity.type == convertIconToDefinition("MagicPlant") then
        ...
        if entity.type == convertIconToDefinition("Mushroom") then
            local pMagic = _PLAYER.stamina
            if pMagic < 20 then
                _PLAYER.stamina = _PLAYER.stamina + 2
            end
